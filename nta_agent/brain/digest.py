...
def digest(state, profile, armies=None, territory=None, decisions=None,
           failures=None, res_pressure=None, lessons=None, leveling=None,
           spares=None) -> dict:
    r = state.resources
    ...
    for a in (armies or []):
        ...
        # Deterministic type signal so the brain/chat identifies an army by its
        # composition instead of guessing: dominant = most common pawn id,
        # pure = made of a single pawn type.
        ...
        army_rows.append({"uid": str(a.get("uid")), "name": a.get("name"),
                          ...
                          "dominant": dominant, "pure": len(comp) == 1})
    ...
    if spares:  # spare armies that can't win anything even together -> player asks the brain
        out["spares"] = spares
