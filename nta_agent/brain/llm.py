...
_SYSTEM = (
    ...
    "army.* (group/roles/composition/strike_target) is the HUMAN's — do not edit it here. "
    "If digest shows the strike-group goal is blocked (a pawn type isn't unlocked, or it "
    "exceeds the army cap), relay that to the player via advice.\n"
...
_CHAT_TOOLS = (
    ...
    "clear the goal (ONE-SHOT). If the player names the new armies ('đặt tên Đội 1 đến Đội "
    ...
    "/ dominated by that shield type), ASK the player back and rename nothing — list the "
    ...
    "NOTE: renames and strike groups are shown to the player for CONFIRMATION before "
    "anything happens, so propose your best mapping clearly; nothing is applied until "
    "they confirm."
)
