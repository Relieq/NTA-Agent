...
_CHAT_TOOLS = (
    ...
    "- rename_plan: [{\"pawn\":<pawn id>,\"name\":\"<new name>\"}] — the PREFERRED way to "
    "rename armies BY TYPE. Just PARSE the request into (pawn type, name) pairs, in the "
    "order the player lists the names; the app itself picks which army each name goes to "
    "(by pure composition) and asks the player back if the counts are ambiguous. Map the "
    "type words to pawn ids: rìu khiên / khiên / tank = 3206; IMP / cường nỏ / nỏ = 3305. "
    "Name <= 12 characters, no newline. Example — 'đổi tên nhóm 5 đội: 1 rìu khiên, 4 IMP "
    "lần lượt Đội 1..Đội 5' -> [{\"pawn\":3206,\"name\":\"Đội 1\"},{\"pawn\":3305,\"name\":"
    "\"Đội 2\"},{\"pawn\":3305,\"name\":\"Đội 3\"},{\"pawn\":3305,\"name\":\"Đội 4\"},"
    "{\"pawn\":3305,\"name\":\"Đội 5\"}].\n"
    "- army_renames: [{\"uid\":\"<uid>\",\"name\":\"<name>\"}] — only when the player "
    "names a SPECIFIC army (by its uid/current name), not by troop type.\n"
    "- question: if the request is unclear (e.g. you can't tell which pawn type the "
    "player means), ASK back and rename nothing. (Type-count ambiguity is handled for "
    "you by the app, so you don't need to check counts yourself.)"
)
