...
- **用备用队伍升级：** 选择队伍组和模式。Agent 提出备用队伍方案（复用零散队伍、合并、征兵、所需经验书/时间），
  由你确认。备用队伍在主城升级后前往主队伍**相邻地块**，**交换同类士兵**——主队伍继续刷怪/挖路
  （少一队时只在不损兵时出击）。
...
**专属装备：**在第 10/18 格由你选择（显示**本局**的随机效果池）。某条期望效果达标后，Agent 会**锁定**该条，
再消耗**固定器**（独立预算）重铸另一条。**熔炼**由你选择副装备、预览并确认，Agent 只发送该指令。
...
**⛏ 挖路到指定地块：** 点击任意地块 → *Dig tới đây*。Agent 规划最快的连续占领路线（行军 + 模拟器给出的战斗时长），
与敌人保持安全距离，并规划据点位置（约每 7 格一个，优先 1 级地），先**预览**格数/时间/体力，
**确认**后由农场队伍开挖，其他队伍照常农场。目标被占会自动改为最近的安全地块；打不过的地块会绕行，
或等待并提示需要接受多少损失。
