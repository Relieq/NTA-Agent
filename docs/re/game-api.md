...
| `HD_SmeltingEquip` | 🔶 | `{mainUid, viceIds:[equipId]}` → `{currSmeltEquip, fixator}` | Dung luyện: món chính chuyên dụng giữ mọi thuộc tính + thêm hiệu ứng món phụ (tối đa 2 ô, Tiệm Rèn Lv14/20; tốn 1 máy cố định mỗi món phụ); xong qua notify 64 SMELT_EQUIP_RET. Chỉ chạy khi người chơi xác nhận trên dashboard. |
| `HD_RestoreSmeltEquip` | 🔶 | `{mainUid}` → `{equip}` | Khôi phục món chuyên dụng về trước khi dung luyện. Chỉ khi người chơi xác nhận. |
